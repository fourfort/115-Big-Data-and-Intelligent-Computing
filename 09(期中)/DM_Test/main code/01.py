...
class ResultVisualizer:
    ...
    def plot_cluster_sentiment_with_topics(self):
        ...
        ax = data.plot(kind='bar', stacked=True,
                       color=['#66b3ff', '#ff9999', '#ffcc99'],
                       edgecolor='black', linewidth=0.5)

        # 強制中文標題
        if chinese_font:
            plt.title('群集情感分佈\n(含自動生成主題關鍵詞)', fontsize=16, fontweight='bold', pad=25, fontproperties=chinese_font)
            plt.xlabel('Cluster (Topic)', fontsize=12, fontproperties=chinese_font)
            plt.ylabel('Count', fontsize=12)
        else:
            plt.title('Cluster Sentiment Distribution\n(with Auto-Generated Keywords)', fontsize=16, fontweight='bold', pad=25)
            plt.xlabel('Cluster (Topic)', fontsize=12)
            plt.ylabel('Count', fontsize=12)

        plt.legend(['Positive', 'Negative', 'Neutral'], title='Sentiment', loc='upper right')

        labels = [f"Cluster {i}\n({cluster_keywords[i]})" for i in data.index]
        plt.xticks(range(len(labels)), labels, rotation=0, ha='center', fontsize=10)
    ...
    def plot_rules_table(self):
        ...
        table = ax.table(cellText=top10.values,
                         colLabels=['前件', '後件', 'Support', 'Confidence'],
    ...
    def plot_classification_comparison(self, clf_results):
        ...
        plt.bar(x - width/2, accuracy, width, label='Accuracy', color='#66b3ff')
        plt.bar(x + width/2, f1_macro, width, label='F1-Macro', color='#ff9999')
        plt.title('Classification Performance Comparison\n(SVM vs Decision Tree)', fontsize=16, fontweight='bold', pad=20)
        plt.xlabel('Model'); plt.ylabel('Score'); plt.xticks(x, models); plt.ylim(0, 1); plt.legend()
        plt.tight_layout()
